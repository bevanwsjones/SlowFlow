...
#g = rp.cartesian_error(cells_matrix, met = 1, phi_function = 2)
...
def uneven_experiment():
    ...
    rp.single_grid_metric(cells_matrix, uneven_matrix, grid_quality = 1, met=1, phi_function = 0)
...
def non_orthogonal_experiment():
    ...
    rp.single_grid_metric(cells_matrix, skew_matrix, grid_quality = 0, met=3, phi_function = 0)
...
def skewness_experiment():
    ...
    rp.single_grid_metric(cells_matrix, stretch_matrix, grid_quality = 2, met=0, phi_function = 0)
...
def nonorth_refine_experiment():
    ...
    # skew_matrix = np.array([[0.0, 0.1], [0.0, 0.3], [0.0, 0.5], [0.0, 0.6], [0.0, 0.7], [0.0, 0.8], [0.0, 0.9]])
    rp.grid_refinement_error(cells_matrix, skew_matrix, grid_quality=0, met = 2, phi_function = 0)
...
def uneven_refine_experiment():
    ...
    rp.grid_refinement_error(cells_matrix, uneven_matrix, grid_quality=1, met = 2, phi_function = 1)

def skewness_refine_experiment():
    cells_matrix = np.array([[3, 3], [9, 9], [30, 30], [45, 45]])
    skew_matrix = np.array([0.1, 0.2, 0.5, 1.0, 1.2])
    rp.grid_refinement_error(cells_matrix, skew_matrix, grid_quality=2, met = 3, phi_function = 0)
